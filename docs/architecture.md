...
Every generated file is written straight into the home its consumer reads it
from. There is no staging tree: an intermediate copy is a second thing that can
be stale, and the copy step that fed it was the only way a client could end up
running a profile that is no longer active. Each file carries a `Generated by
ailocal. Do not edit.` header naming its source.
...
Deleting either root and re-running `ailocal start` recovers everything
generated. Deleting `profiles/` or `.env` does not: those are authored.
...
Shipped assets are mounted read-only straight out of the wheel, so ailocal
never writes to a directory a running container is mounted on. Generation
always completes before `docker compose up`, and nothing regenerates a mounted
asset while the stack runs.
