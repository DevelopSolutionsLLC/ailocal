...
# Generated wrappers live with every other generated artefact (ADR 009).
APP_SUPPORT="${AILOCAL_STATE:-$(python3 "$ROOT_DIR/lib/profile-config" state-root)}/agents"
...
# The agent must never carry a BAKED model tag. It did, and the installed
# agent went on warming a model the profile had long since replaced: the tag
# was not pulled, curl failed, and the agent sat at exit 22 with nobody
# looking. Resolve the backend and its TTL at RUN time, through the installed
# command, so the agent cannot drift from the active profile.
AILOCAL_BIN="$(command -v ailocal || true)"
if [ -z "$AILOCAL_BIN" ]; then
  warn "ailocal is not on PATH; the login agent would have to name a checkout path"
  warn "Install the command first, then re-run: ailocal autostart"
  exit 1
fi
PRELOAD="$APP_SUPPORT/preload.sh"
cat > "$PRELOAD" <<WRAP
#!/bin/sh
# Generated by 'ailocal autostart'. Resolves the model at run time.
O="http://127.0.0.1:11434"
for _ in \$(seq 1 60); do curl -fsS -m 3 "\$O/api/version" >/dev/null 2>&1 && break; sleep 2; done
curl -fsS -m 3 "\$O/api/version" >/dev/null 2>&1 || exit 0   # Ollama never came up
M="\$("$AILOCAL_BIN" profile role $MODEL_ROLE --field model 2>/dev/null)"
[ -n "\$M" ] || exit 0                                        # unresolvable: stay quiet
K="\$("$AILOCAL_BIN" profile role $MODEL_ROLE --field keep_alive 2>/dev/null)"
[ -n "\$K" ] || K=-1
case "\$K" in -1) KJ=-1 ;; *) KJ="\"\$K\"" ;; esac       # Go duration: -1 bare, "6h" quoted
curl -fsS -m 5 "\$O/api/ps" 2>/dev/null | grep -q "\"\$M\"" && exit 0   # already resident
...
info "primary model '$MODEL_ROLE' preloads at login, resolved at run time"
