...
#
# Usage:
#   ailocal preload              # preload the default backend (coder)
#   ailocal preload architect     # preload a different role's backend
#   ailocal preload --now        # just warm now, don't install the agent
#   ailocal preload --uninstall  # remove the LaunchAgent
#
...
resolve_backend() {
  ...
  local resolved; resolved="$(python3 "$ROOT_DIR/lib/sync-models.py" --resolve "$want" 2>/dev/null)"
...
<dict>
  ...
  <array>
    <string>$ROOT_DIR/lib/preload-model.sh</string>
    <string>--now</string>
    <string>$ROLE</string>
  </array>
  <key>RunAtLoad</key>
...
echo
echo "  Change model:   ailocal preload <role>"
echo "  Remove:         ailocal preload --uninstall"
