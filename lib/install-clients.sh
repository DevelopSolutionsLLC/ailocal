...
install_ailocal_launcher() {
  ...
  cat > "$shim" <<'SHIM'
#!/usr/bin/env bash
# GENERATED by ailocal lib/install-clients.sh — do not edit.
# Canonical source: <repo>/ailocal ; repo location: ~/.config/ailocal/repo
set -euo pipefail
...
root="$(cat "$cfg/repo")"
if [[ ! -x "$root/ailocal" ]]; then
  echo "ailocal: repo recorded at '$root' has no ailocal." >&2
