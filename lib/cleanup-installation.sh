#!/usr/bin/env bash
# cleanup-installation.sh — act on lib/audit-installation.sh findings.
#
...
BACKUP="$AILOCAL_STATE/backups/cleanup-$STAMP"
...
if [ ! -s "$FINDINGS" ]; then
  ...
  bash lib/audit-installation.sh >/dev/null 2>&1 || true
fi
if [ ! -s "$FINDINGS" ]; then
...
if [ "$MODE" = dry ]; then
  ...
  echo " Re-run ailocal audit to confirm."
fi
