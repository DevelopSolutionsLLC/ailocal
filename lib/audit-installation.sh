...
#
# Companion: lib/cleanup-installation.sh acts on these findings, behind
...
if command -v code >/dev/null 2>&1; then
  if code --list-extensions 2>/dev/null | grep -qix "Gethnet.litellm-connector-copilot"; then
    ...
    flag MISSING "connector extension" "VS Code" "run ailocal vscode"
  fi
else
...
if found:
    ...
    print("      action:   ailocal vscode (prunes them, backs up first)")
else:
...
python3 lib/sync-models.py >/dev/null 2>&1
...
echo "     ailocal cleanup --dry-run"
echo "     ailocal cleanup --apply     # backs up first"
exit 3
