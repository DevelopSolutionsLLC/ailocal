...
#
#   ailocal status          UNIFIED DASHBOARD (ailocal status)
#   ailocal status --models verbose per-capability view (the previous default)
#   ailocal status --table  compact capability table    (ailocal models)
#
...
if [ "$MODE" = "dashboard" ]; then
  ...
  docker logs --since 2h ailocal-litellm 2>/dev/null \
    | python3 "$ROOT_DIR/lib/status_gateway.py" 2>/dev/null
  ...
  hdr "Recent requests"
  if [ -d "$TRACES" ]; then
    python3 "$ROOT_DIR/lib/status_traces.py" "$TRACES"
  else
