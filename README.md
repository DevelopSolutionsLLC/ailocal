...
**VS Code: model spins on "Considering…" and never answers** — you selected a thinking model (`reasoner`) whose reasoning streams invisibly; the connector shows "Considering…" until it finishes, which can be minutes. Use `coder` for direct answers, or `deep` for visible thinking (its reasoning is merged into the answer text). If you hit "Message exceeds token limit," the model's context is smaller than Copilot's agent prompt — pick a role with a larger window (`coder` is 256K, `supervisor`/`deep` 128K, `router` 64K). (A persistent 401 with "Ensure Key has Bearer prefix" instead means the connector's API key isn't entered — re-enter it via **Chat: Manage Language Models**.)
